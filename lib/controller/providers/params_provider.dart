...
class ParamsProvider with ChangeNotifier {
  bool isLoaded = false;
  Map? paramsDataPro;
  bool isTapped = false;
  ...
  List selectedParams = [
    ...
    {"p_name": "Outside Octroi Limits", "selected_val": ""},//lec_remaks
  ];
  ...
  Future<void> loadRepoInProvider(
    ...
    if (paramsDataPro == null) {
      ...
      print('lecParameter ID ---103------$lecParameterId');

      setIsLoaded(false);
    }else if(paramsDataPro !=null){
      // here we get a id from a map
    //  lecParameterId = ('${paramsDataPro!["lec_parameter_id"]}');

      print('lecParameter ID ---113----$lecParameterId');
  ...
  Future<void> fetchParamsInProvider(
    ...
    debugPrint("status: ${fpmap['status']} lec_parameter_id: ${fpmap['data'][0]['lec_parameter_id']} ");
    ...
    Future.delayed(Duration(seconds: 2), () {
      if (fpmap['status'] == true) {
        ...
        setParamsValue(12, fpmap['data'][0]['lec_remaks']!);
        setParamsValue(13, fpmap['data'][0]['lec_report_upload']!);
        setParamsValue(14, fpmap['data'][0]['lec_report_upload']!);
      }
    });
